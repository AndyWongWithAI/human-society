...
- [ ] **数据源独立性核查（跨源锚点）**：L4 的复合锚点若涉及经济/统计指标，核查各指标的底层测量链是否独立。不得将同一套国家账户数据的不同发布机构当作"多源验证"。见 `sources/independence-model.yaml` 的 `data_source_origin` 节。
...
- [ ] **工具可及性诚实标注**：若 L4 锚点的数据来自 API/MCP 工具调用，诚实标注工具可靠性分层（稳定/有限流/不可用），不得将工程可及性混同于认识论独立性。
